...
import { Search, Plus, ChevronRight, UserPlus, Check, X, Ghost } from 'lucide-react';
  ...
  return (
    <div className="p-6 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-32">
      ...
      <div className="relative group">
        ...
        <input 
          type="text"
          placeholder="Search vibes or tags..."
          className="w-full glass rounded-[28px] py-5 pl-12 pr-12 text-white focus:ring-2 focus:ring-violet-500 outline-none transition-all placeholder:text-zinc-700 font-bold tracking-tight shadow-lg"
          value={search}
        ...
        {search && (
          <button 
            onClick={clearSearch}
            className="absolute right-4 top-1/2 -translate-y-1/2 p-2 hover:bg-white/10 rounded-full transition-all text-zinc-500 hover:text-white"
          >
            <X size={18} />
          </button>
        )}
      ...
      <div className="grid grid-cols-1 gap-5">
        {filtered.length > 0 ? (
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-1.5 mb-2">
                    {guild.tags.map(tag => (
                      <span key={tag} className="text-[8px] px-2 py-0.5 bg-white/5 rounded-full text-zinc-500 font-bold uppercase tracking-widest border border-white/5">
                        #{tag}
                      </span>
                    ))}
                  </div>
          ...
          <div className="flex flex-col items-center justify-center py-20 px-6 text-center animate-in fade-in zoom-in duration-500">
            <div className="w-24 h-24 bg-zinc-900 rounded-[32px] flex items-center justify-center text-zinc-700 mb-6 border border-zinc-800 shadow-inner">
              <Ghost size={40} />
            </div>
            <h3 className="text-xl font-black text-zinc-200 uppercase tracking-tighter mb-2">No Guild Found</h3>
            <p className="text-sm text-zinc-500 font-bold max-w-[200px] leading-relaxed">
              The vibe "{search}" hasn't been manifested yet. Maybe start it?
            </p>
            <button 
              onClick={clearSearch}
              className="mt-6 text-xs font-black uppercase tracking-widest text-violet-400 hover:text-violet-300 underline underline-offset-4"
            >
              Show all guilds
            </button>
          </div>
